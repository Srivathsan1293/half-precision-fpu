// tb/tb_fdiv_seq.cpp
//
// Verification for the sequential-SRT DIV (src/fpu_FDIV.sv + srt in
// src/fdiv_datapath_blocks.sv). The divider is non-pipelined: a one-cycle
// `start` pulse latches the operands and the SRT runs a fixed 11-cycle
// schedule, capturing one result per `done` pulse.
//
// Handshake model: present each test with a one-cycle `start` pulse, then wait
// for `done` (11 cycles later); the DIV captures the quotient at the done
// edge, so the answer is sampled on the following cycle. One test completes
// every 12 cycles.
//
// Golden = native IEEE-754 _Float16 division + the RTL special-case overrides
// (NaN / 0/0 / Inf/Inf -> 0x7E00; Inf/B0 -> 0x7C00; A0/BInf -> +/-0).
...
int main(int argc, char** argv) {
    ...
    if (argc > 1) SAMPLES = strtoull(argv[1], nullptr, 10);
    ...
    for (uint16_t a : edges) for (uint16_t b : edges) tests.push_back({a, b});

    srand(12345);
    for (uint64_t i = 0; i < SAMPLES; i++)
        tests.push_back({(uint16_t)(rand() & 0xFFFF), (uint16_t)(rand() & 0xFFFF)});

    std::ofstream log("testing_results/fdiv_seq_failures.log");

    auto check = [&](uint16_t a, uint16_t b, uint16_t got, uint64_t& n_fail,
                     uint64_t cat_fail[5], uint64_t cat_tot[5]) {
    ...
    uint64_t n_fail = 0, cat_fail[5] = {0}, cat_tot[5] = {0};

    bool wait_done = false;
    size_t n_presented = 0;
    uint64_t n_checked = 0;

    while (n_checked < tests.size()) {
        ...
        check(tests[n_presented - 1].first, tests[n_presented - 1].second,
              dut->ans, n_fail, cat_fail, cat_tot);
        n_checked++;
    }

    std::cout << "tests=" << n_checked << " fails=" << n_fail << "\n";
