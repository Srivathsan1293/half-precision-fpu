...
//   7. pcpi_rd[15:0] holds the correct FPU result (sampled after the
//      4-cycle pipeline fill): FADD(1.0, 2.0) = 3.0 = 0x4200
//   8. No re-trigger: while pcpi_valid stays high after the ready pulse,
//      pcpi_wait/pcpi_ready/pcpi_wr must stay low (spec §2.2 req 4)
...
static Handshake run_handshake(Vfpu_pcpi* dut, uint32_t insn,
    ...
    for (int cyc = 0; cyc < 16; cyc++) {
        ...
        if (h.ready_first >= 0 && cyc > window_end) {
            if (o.wait || o.ready || o.wr) h.retrigger = true;
        }
        if (o.ready) {
...
int main(int argc, char** argv) {
    ...
    Obs first = cycle(dut, 1, 1, insn_fadd, rs1, rs2);   // accept cycle
    check("accept cycle: pcpi_ready is LOW (no immediate commit)", !first.ready);
